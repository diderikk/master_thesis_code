...
# Reads CSV file generates by resource_scraper.py
...
# Generates time series plot of the resource consumption metrics consisting of all services
	...
	for i, pod in enumerate(pods.keys()):
		for j, (metric_name, metric_unit, metric_title) in enumerate(metrics_names):
			...
			for service_name, data_frame in data_frames:
				...
				# Reset timestamp value, so all time series occur at the same time
				if min_time is None:
...
# Generates a table of resource consumption metrics from all services in LaTeX and Matplotlib formats
def generate_table(data_frames: list, pods: dict, metrics_names: list):
	...
	for metric_name, metric_unit, metric_title in metrics_names:
		columns.extend([f'{metric_title} Mean', f'{metric_title} p70', f'{metric_title} p90',
			f'{metric_title} Max'])
	for service_name, data_frame in data_frames:
		for pod in pods.keys():
			curr_data = []
...
# Generates aggregated statistics bar graphs using metrics from all services
...
if __name__ == '__main__':
	...
	for test_type in test_types:
		if test_type == "external":
			# Relevant pods 
			# pods = {'simple': 'HTTP Server', 'database': 'Database', 'traefik-mesh-proxy': 'Proxy', 'ztunnel': 'Proxy'}
			# Worker nodes
			pods = {'worker': 'Node Total'}
		else:
			# Relevant pods 
			# pods = {'distributed-k6-test': 'HTTP Client', 'simple': 'HTTP Server', 'database': 'Database', 'traefik-mesh-proxy': 'Proxy', 'ztunnel': 'Proxy'}
			# Worker nodes
			pods = {'worker': 'Node Total'}
		...
		for load_type in load_types:
			...
			pods['total'] = 'Total'
			# Combines all the metrics from all the relevant pods into a single total metric
			# Time series
			generate_plots(data_frames, pods, metrics)
			...
			# Table format
			save_to_file(generate_table(data_frames, pods, metrics), f'./new_plots/resource_consumtion/{test_type}/{load_type}_latex_total_table.txt')
			plt.clf()
