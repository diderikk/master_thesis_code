...
def generate_table(file_paths: list, metric_name: str):
    columns = ['Target VUS', 'Mean VUS', 'Median (ms)',
    ...
    for name, file_path in file_paths:
        rows.append(name)
        data.append(generate_row(file_path, metric_name))
...
if __name__ == '__main__':
    test_types = ['internal']
    load_types = ['spike'] # 'stress200','stress500','stress700','spike'
    ...
    for test_type in test_types:
        for load_type in load_types:
            ...
            #     plt.tight_layout()
            # plt.savefig(f'./plots/request/{test_type}/{load_type}_VUs_graph.png')
            # plt.clf()

            # for i, metric_name in enumerate(metric_names):
            #     plt.figure(i + len(metric_names), figsize=(15, 6))
            #     save_to_file(generate_table(file_paths, metric_name), f'./plots/request/{test_type}/{load_type}_latex_table.txt')
            #     plt.tight_layout()
            # plt.savefig(f'./plots/request/{test_type}/{load_type}_table.png')
            # plt.clf()
