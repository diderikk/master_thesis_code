...
if __name__ == '__main__':
	test_types = ['internal']
	load_types = ['smoke', 'stress', 'stress700'] 
	...
	for test_type in test_types:
		for load_type in load_types:
			...
			# save_to_file(generate_table(data_frames), f'./plots/trace/{test_type}/{load_type}_latex_table.txt')
			# # plt.tight_layout()
			# plt.savefig(f'./plots/trace/{test_type}/{load_type}_table.png')
			# plt.clf()
			print(f"Generating plot for {test_type}, {load_type}")
			generate_bar_graph(data_frames)
			plt.tight_layout()
			plt.savefig(f'./plots/trace/{test_type}/{load_type}_bar_graph.png')
			plt.clf()
