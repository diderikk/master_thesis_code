...
func attachEbpf(objs *programObjects) {
	...
	go func() {
		...
		for {
			...
			log.Printf("Packet stopped by eBPF program. Source IP address: %s, Destination IP address: %s\n", formatIp(event.Saddr), formatIp(event.Daddr))
		}
	}()

	attachProgramToInterfacesLoop(objs.PolicyCheck, netlink.HANDLE_MIN_INGRESS)
}
	...
	for _, iface := range ifaces {
		...
		for _, filter := range filters {
			if bpfFilter, ok := filter.(*netlink.BpfFilter); ok {
				if bpfFilter.Name == "network_policy" {
					pass = true
				}
			}
		}
